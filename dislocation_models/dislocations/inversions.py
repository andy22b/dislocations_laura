...
class EdgecumbeInversion:
    ...
    def fitness(self, x: np.ndarray):
        ...
        edge_contribution = np.linalg.norm(self.combined_edge * x) * self.combined_edge_weight

        combined_fitness = disp_contribution + laplacian_contribution + edge_contribution
        return np.array([combined_fitness])

class HundaleeInversion:
    def __init__(self, laplacian: np.ndarray, edge_array: np.ndarray,
                 lidar_gfs: np.ndarray, lidar_displacements: np.ndarray,
                 oic_gfs: np.ndarray, oic_displacements: np.ndarray,
                 bathymetry_gfs: np.ndarray, bathymetry_displacements: np.ndarray,
                 lidar_weight: float, oic_weight: float, bathymetry_weight: float,
                 laplacian_weight: float, edge_weight: float, max_slip: float = 8.):
        assert all([isinstance(a, np.ndarray) for a in [laplacian, edge_array,
                                                        lidar_gfs, lidar_displacements,
                                                        oic_gfs, oic_displacements,
                                                        bathymetry_gfs, bathymetry_displacements]])
        assert all([arr.ndim == 2 for arr in [laplacian, lidar_gfs, oic_gfs, bathymetry_gfs]])
        assert all([a.ndim == 1 for a in [edge_array, lidar_displacements, oic_displacements, bathymetry_displacements]])

        self.laplacian = laplacian
        self.edge_array = edge_array
        self.lidar_gfs = lidar_gfs
        self.lidar_displacements = lidar_displacements
        self.oic_gfs = oic_gfs
        self.oic_displacements = oic_displacements
        self.bathymetry_gfs = bathymetry_gfs
        self.bathymetry_displacements = bathymetry_displacements
        self.lidar_weight = lidar_weight
        self.oic_weight = oic_weight
        self.bathymetry_weight = bathymetry_weight
        self.laplacian_weight = laplacian_weight
        self.edge_weight = edge_weight
        self.max_slip = max_slip

        self.num_patches = len(self.edge_array)


    def get_bounds(self):
        return [-1. * float(self.max_slip)] * self.num_patches, [float(self.max_slip)] * self.num_patches

    def gradient(self, x):
        return pg.estimate_gradient(lambda x: self.fitness(x), x)

    def fitness(self, x: np.ndarray):
        lidar_contribution = self.lidar_weight * np.linalg.norm(np.matmul(self.lidar_gfs, x) - self.lidar_displacements)
        oic_contribution = self.oic_weight * np.linalg.norm(np.matmul(self.oic_gfs, x) - self.oic_displacements)
        bathymetry_contribution = self.bathymetry_weight * np.linalg.norm(np.matmul(self.bathymetry_gfs, x) - self.bathymetry_displacements)
        laplacian_contribution = self.laplacian_weight * np.linalg.norm(np.matmul(self.laplacian, x))
        edge_contribution = self.edge_weight * np.linalg.norm(self.edge_array * x)
        combined_fitness = lidar_contribution + oic_contribution + bathymetry_contribution + laplacian_contribution + edge_contribution
        return np.array([combined_fitness])
